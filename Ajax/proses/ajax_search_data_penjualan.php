...
<table>
      ...
      <tbody>
        ...
        $result = mysqli_query($koneksi,"SELECT * FROM tbl_penjualan WHERE kd_barang LIKE '%$cari%' OR nm_barang LIKE '%$cari%' ORDER BY kd_barang");
        $total = mysqli_num_rows($result);
        $pages = ceil($total/$halaman);            
        $query = mysqli_query($koneksi,"SELECT * FROM tbl_penjualan WHERE kd_barang LIKE '%$cari%' OR nm_barang LIKE '%$cari%' LIMIT $mulai, $halaman")or die(mysqli_error());
        $no =$mulai+1;
        }
       ...
       while ($tampil = mysqli_fetch_array($query)) {
                ...
                <tr>
                    ...
                    <td><?php echo "Rp.".number_format($tampil['total_harga']).",-"; ?></td>
                </tr>
                <?php 
                }
                ?>
